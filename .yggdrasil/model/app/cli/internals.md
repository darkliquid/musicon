...
- supports a startup flag that points Musicon at an explicit TOML config path, otherwise falling back to the default user XDG config location exposed by `internal/config`
...
- injects queue, playback, and visualization services from the runtime into `ui.Services` plus typed UI options derived from config, including `ui.compact_mode`, the `[keybinds]` section including the compact toggle, a resolved semantic `[ui.theme]` palette, and queue bindings for visible search-kind slot selection, search-kind cycling, and collection expansion
- resolves an XDG-style session-state file (`$XDG_STATE_HOME/musicon/session.json` or `~/.local/state/musicon/session.json`), loads the saved UI/runtime snapshot before constructing the UI, restores the audio engine from that snapshot without auto-starting playback, and passes an app-owned JSON session store into `ui.NewApp(...)`
...
The package source now also carries package-level and exported-symbol documentation so CLI wiring, source aggregation, and resolver routing remain legible through Go tooling without tracing every startup path manually. The repo-wide contributor-doc pass also added higher-level file comments around the startup pipeline so newcomers can see where configuration, source construction, runtime construction, and UI handoff happen.
...
- Chose an app-owned `--config-path` startup flag over teaching every runtime package about config-file lookup because explicit config-path selection is a process-start concern that belongs in the executable wiring layer.
...
- Chose to translate compact-mode startup preference in the CLI layer alongside the rest of the typed UI options because the executable already owns config precedence and session restore wiring.
- Chose to resolve `[ui.theme]` into a semantic palette in the CLI/config layer before constructing `internal/ui` because palette-file loading and config precedence are application concerns, while the UI should only consume resolved color roles.
...
- Chose an app-owned JSON session file under the user's XDG state directory over adding more config fields or letting the UI write directly to disk because the user explicitly wanted reopening Musicon to feel seamless without making durable storage policy a presentation concern.
