...
- injects queue, playback, and visualization services from the runtime into `ui.Services` plus typed UI options derived from config, including the `[keybinds]` section, a resolved semantic `[ui.theme]` palette, and queue bindings for visible search-kind slot selection, search-kind cycling, and collection expansion
