...
Queue mode arranges source chips, filter chips, search input, and a single merged browser list directly inside the square viewport without adding another persistent panel wrapper. Queued items stay pinned at the top of that browser with a marker, while current search results are appended below them without the marker until the user adds them to the queue. The root model now builds Bubble-style key bindings from typed options, passes the queue-specific map into the queue screen, and passes the shared browser-navigation bindings down into the reusable `pkg/components.List`. Queue search now has an explicit focus toggle: when search is focused, printable keys go to the query and printable navigation aliases such as `j`/`k` do not leak into the browser; when search is unfocused, queue-management shortcuts such as source switching, filter toggles, removal, and reorder actions become active again. Source-backed searches still run through Bubble Tea commands instead of inline in the key handler, which keeps quit, help, mode switching, and browser movement responsive even when a source is slow or networked, and the queue browser flips into its searching empty state as soon as a remote lookup starts so slow providers no longer look idle. It now also polls the playback snapshot while rendering so the active queued track can be marked inline, and it uses dedicated reorder keys to move queued items while keeping the selected row attached to the moved entry.
