...
The package source now also carries package-level and exported-symbol documentation so shared widget contracts remain visible through Go docs alongside the existing implementation-level notes. The contributor-doc sweep also added file-level overview comments to the list, layout, and image implementation files so newcomers can separate shared geometry rules from widget behavior and terminal-rendering concerns.
